...
@click.option('-m', '--debug-mail', default=False, help='Dump debug info about emails that the app tries to send', is_flag=True)
@click.option('-r', '--resume', default=False, help='Resume from the last test which failed', is_flag=True)
...
def compile_less_to_css(less_file, css_file):
    ...
    try:
        subprocess.check_call(['../../node_modules/.bin/lessc', '--source-map', '--clean-css', less_file, css_file])
